...
CREATE OR REPLACE FUNCTION test_trigger_schema.mock_http_post(
    ...
    headers JSONB,
    request_body TEXT
) RETURNS TABLE(id bigint, status int, response_body jsonb) AS $$
DECLARE
...
BEGIN
    ...
    request_payload := request_body::JSONB;
    ...
    -- Calculate current attempt number by counting previous calls to this function
    SELECT COUNT(*) 
    FROM test_trigger_execution_logs 
    WHERE trigger_name = 'mock_http_function' 
    AND row_id = conversation_id
    INTO attempt_count;
    ...
    IF conversation_id = 101 THEN
        RETURN QUERY SELECT 
            conversation_id + 1000 AS id, 
            200 AS status, 
            '{"message":"Success simulation for test case 101"}'::jsonb AS response_body;
    
    -- For conversation_id 102: Fail first attempt, succeed on second
    ELSIF conversation_id = 102 THEN
        -- Check if this is the first or subsequent attempt
        ...
        ELSE
            -- Second or later attempt: Return success
            RETURN QUERY SELECT 
                conversation_id + 1000 AS id, 
                200 AS status, 
                '{"message":"Success on retry for test case 102"}'::jsonb AS response_body;
        END IF;
    ...
    ELSE
        RETURN QUERY SELECT 
            conversation_id + 1000 AS id, 
            200 AS status, 
            '{"message":"Default success response"}'::jsonb AS response_body;
    END IF;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    IF NEW.elevenlabs_conversation_id IS NOT NULL AND NEW.elevenlabs_conversation_id != '' AND NEW.status IS DISTINCT FROM 'processed' THEN
        ...
        WHILE retry_count < max_retries AND NOT call_success LOOP
            ...
            BEGIN
                -- Use our mock HTTP function with proper column types
                SELECT id, status, response_body
                FROM test_trigger_schema.mock_http_post(
                    url := edge_function_url,
