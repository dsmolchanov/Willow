-- Fix for the execution_time type mismatch in the trigger function
...
BEGIN
  ...
  -- Store the current timestamp directly instead of a duration
  ...
  VALUES (
    ...
    TG_OP, 
    NEW.conversation_id, 
    clock_timestamp(),  -- Just use the current timestamp
    jsonb_build_object(
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'status', COALESCE(NEW.status, 'unknown'),
      'elevenlabs_conversation_id', NEW.elevenlabs_conversation_id,
      'elapsed_ms', EXTRACT(EPOCH FROM (clock_timestamp() - start_time)) * 1000  -- Store elapsed time in milliseconds in the details
    )
  );

  -- Safely try multiple approaches to make the HTTP call
  BEGIN
    -- First attempt: Try with Supabase HTTP wrapper function if available
    BEGIN
      SELECT * 
      INTO http_response
    ...
    EXCEPTION WHEN undefined_function THEN
      -- Second attempt: Try with pg_net extension named parameters
      BEGIN
        SELECT * 
        INTO http_response
        FROM net.http_post(
      ...
      EXCEPTION WHEN undefined_function THEN
        -- Third attempt: Try with pg_net positional parameters
        BEGIN
          SELECT * 
          INTO http_response
          FROM net.http_post(
        ...
        EXCEPTION WHEN undefined_function THEN
          ...
          VALUES (
            ...
            clock_timestamp(),  -- Just use the current timestamp
            jsonb_build_object(
              'error', 'Could not find any HTTP function to call the edge function',
              'message', 'Check that pg_net extension is installed and Supabase configuration is correct',
              'elapsed_ms', EXTRACT(EPOCH FROM (clock_timestamp() - start_time)) * 1000  -- Store elapsed time in milliseconds in the details
            )
          );
