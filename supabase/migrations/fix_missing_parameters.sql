-- Updated trigger with complete parameter set
...
BEGIN
    ...
    IF NEW.elevenlabs_conversation_id IS NOT NULL AND NEW.elevenlabs_conversation_id != '' AND NEW.status IS DISTINCT FROM 'processed' THEN
        ...
        IF NEW.end_time IS NOT NULL THEN
            request_payload := request_payload || jsonb_build_object('end_time', NEW.end_time);
        ELSIF NEW.updated_at IS NOT NULL THEN
            -- Fall back to updated_at if end_time is not available
            request_payload := request_payload || jsonb_build_object('end_time', NEW.updated_at);
        ELSE
            -- Use current timestamp as last resort
            request_payload := request_payload || jsonb_build_object('end_time', CURRENT_TIMESTAMP);
        END IF;
        ...
        INSERT INTO trigger_execution_logs (
            ...
            CURRENT_TIMESTAMP,
            jsonb_build_object(
                'debug', 'Calling edge function with complete parameters',
